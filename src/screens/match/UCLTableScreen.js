...
const UCLTableScreen = ({route, navigation}) => {
  ...
  const getStandings = () => {
    ...
    getLeagueRankings(leagueId, season)
      .then(res => {
        setRankings(res.data.response[0].league.standings);
        setIsLoading(false);
      })
      .catch(err => {
        console.log(err);
  ...
  return (
    <ScrollView style={styles.page}>
      {rankings.map(groupRankings => (
        <View>
          <View style={styles.groupItem}>
            <Text style={styles.groupTitle}>{groupRankings[0]?.group}</Text>
          </View>
